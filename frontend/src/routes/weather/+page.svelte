...
<div class="space-y-6">
	...
	{:else if weather}
		...
		<div class="rounded-xl border border-border bg-surface p-6">
			<h2 class="mb-4 text-xl font-bold capitalize">{weather.city}, {weather.country}</h2>
			<div class="flex flex-wrap items-center gap-8">
				<div>
					<div class="text-5xl font-bold">{weather.current.temperature_c}&deg;C</div>
					<div class="mt-1 text-sm text-text-muted capitalize">{weather.current.condition}</div>
				</div>
				<div class="space-y-2 text-sm">
					<div class="flex items-center gap-2 text-text-muted">
						<Thermometer size={16} />
						<span>Feels like {weather.current.feels_like_c}&deg;C</span>
					</div>
					<div class="flex items-center gap-2 text-text-muted">
						<Droplets size={16} />
					...
					<div class="flex items-center gap-2 text-text-muted">
						<Wind size={16} />
						<span>Wind {weather.current.wind_speed_kmh} km/h {weather.current.wind_direction}</span>
					</div>
				</div>
			</div>
		</div>
					...
					{#each weather.forecast as day}
						<div class="rounded-xl border border-border bg-surface p-4 text-center">
							...
							<div class="flex items-center justify-center gap-2">
								<span class="font-semibold">{day.high_c}&deg;</span>
								<span class="text-text-muted">{day.low_c}&deg;</span>
							</div>
							<div class="mt-1.5 text-xs text-text-muted">
								<Droplets size={12} class="inline" /> {day.precipitation_chance}%
							</div>
						</div>
					{/each}
				</div>
