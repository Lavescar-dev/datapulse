			...
			<div class="rounded-xl border border-border bg-surface p-5">
				<div class="mb-3 flex items-center justify-between">
					...
					<div class="flex items-center gap-1 text-sm font-medium"
						class:text-success={crypto.change_24h_percent >= 0}
						class:text-danger={crypto.change_24h_percent < 0}
					>
						{#if crypto.change_24h_percent >= 0}
							<ArrowUpRight size={16} />
						{:else}
							<ArrowDownRight size={16} />
						{/if}
						{crypto.change_24h_percent >= 0 ? '+' : ''}{crypto.change_24h_percent.toFixed(2)}%
					</div>
				</div>

				<div class="mb-3 text-2xl font-bold">${crypto.current_price.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</div>

				{#if crypto.sparkline?.length}
					<div class="mb-3">
						<SparklineChart
							data={crypto.sparkline}
							color={crypto.change_24h_percent >= 0 ? '#22c55e' : '#ef4444'}
							width={200}
							height={40}
						/>
					</div>
